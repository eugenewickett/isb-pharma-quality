...
def SupAsymPrices(b, c, cSup):
    ...
    # Uses first-order conditions
    w1 = max(((b-1) * (b+2) * (c-1) + (b*cSup)) / (4-b**2), 0)
    w2 = max(((b-1) * (b+2) * (c-1) + (2*cSup)) / (4-b**2), 0)
    return w1, w2

def SupAsymLimitPrices(b, c, cSup, retQualInvest, lambsuphi, lambsuplo, lambrethi, lambretlo, LthetaR, LthetaS, sens,
                       fpr, numWpts=100, printUpdate=False):
    # Returns wholesale prices under asymmetric supplier quality levels, assuming S1 is low-quality
    # Uses limit price set by S1 that is sufficiently low to dissuade strategy change in either retailer or S2
    # First-order condition w1; limit price w1 will not exceed this
    if retQualInvest is True:
        lambret = lambrethi
        retCurrStrat = 0
    else:
        lambret = lambretlo
    ...
    for w1curr in wVec:
        if w1curr <= w1max:
            validLimit = True  # Initial value
            sup2utilArr = np.zeros((wVec.shape[0], 2))
            retStratArr = np.zeros((wVec.shape[0], 2)) - 1
            # Get the utility of each possible price/quality response of S2
            for w2currInd, w2curr in enumerate(wVec):
                for s2utilInd in [0, 1]:
                    lambsup2 = s2utilInd*lambsuphi + (1-s2utilInd)*lambsuplo
                    # Get retailer policy response
                    currRetUtilVec = UtilsRet_Scen5(lambsuplo, lambsup2, LthetaR, b, c, w1curr, w2curr, lambretlo,
                                                    lambrethi, sens, fpr)
                    currRetStrat = np.argmax(currRetUtilVec)
                    q1curr, q2curr = RetOrderQuantsFromStrat(currRetStrat, b, c, w1curr, w2curr)
                    sup2utilArr[w2currInd, s2utilInd] = SupUtil(q2curr, w2curr, cSup, LthetaS, lambret, lambsup,
                                                                sens, fpr)
                    retStratArr[w2currInd, s2utilInd] = currRetStrat
            # Identify highest utility for S2
            (w2bestind, s2qualind) = np.unravel_index(sup2utilArr.argmax(), sup2utilArr.shape)
            if s2qualind == 0:  # S2 wants to reduce quality
                validLimit = False
            if retStratArr[w2bestind, s2qualind] != retCurrStrat:  # Retailer is switching strategies
                validLimit = False
            if validLimit is True:
                w1lim = w1curr
                w2lim = wVec[w2bestind]
                if printUpdate is True:
                    currRetUtilVec = UtilsRet_Scen5(lambsuplo, lambsuphi, LthetaR, b, c, w1lim, w2lim, lambretlo,
                                                    lambrethi, sens, fpr)
...
def SupMinPriceSing(c, cSup, retQualInvest, sQualInvest, lambsuphi, lambsuplo, lambret, LthetaS, sens, fpr, eps=0.01):
    ...
    if sQualInvest == 1:
        ...
        lambsup = lambsuplo
    # Check utility under single-sourcing; if negative, then positive utility is impossible
    w_sing = (1-c*retQualInvest+cSup*sQualInvest)/2  # Util-maximizing w for supplier under single-sourcing
    q_sing = (1-c*retQualInvest-w_sing)/2
    if SupUtil(q_sing, w_sing, cSup*sQualInvest, LthetaS, lambret, lambsup, sens, fpr) < 0:  # Only negative utility possible
        w_min = 0.0
    else:
        def SupUtilForSolve(w):
            return SupUtil((1-c*retQualInvest-w)/2, w, cSup * sQualInvest, LthetaS, lambret, lambsup, sens, fpr)
        w_min = scipyOpt.newton(SupUtilForSolve, 0)  # Initialize at zero; otherwise it might try to give a very large w
        if w_min > 0.99:
            print('Problem: w_min greater than 1')
    return w_min + eps

def SupExclPrices(b, c, cSup, s1QualInvest, retQualInvest, lambsuphi, lambsuplo, lambrethi, lambretlo, LthetaR, LthetaS,
                  sens, fpr, numWpts=100, printUpdate=False):
    ...
    if s1QualInvest is True:  # S1 makes quality investment
        ...
        lambsup1 = lambsuphi
    # Initialize best w1 as price where utility is zero
    bestw1_retInv_supInv = SupMinPriceSing(c, cSup, 1, s1QualInvest, lambsuphi, lambsuplo, lambrethi, LthetaS, sens, fpr)
    bestw1_noRetInvest_supInv = SupMinPriceSing(c, cSup, 0, s1QualInvest, lambsuphi, lambsuplo, lambretlo, LthetaS, sens, fpr)
    bestw1 = min(bestw1_retInv_supInv, bestw1_noRetInvest_supInv)

    optw1 = (1-c*retQualInvest+cSup*s1QualInvest)/2

    if printUpdate is True:
        print('Seeking limit price...')
    for w1 in wVec:
        if w1>bestw1 and w1<=optw1:
            validLimit = True
            for w2 in wVec:
...
def WholesalePricesFromEq(eqind, b, c, cSup, lambsuplo, lambsuphi, lambretlo, lambrethi, LthetaR, LthetaS, sens, fpr,
    ...
    if eq == 0:  # {HHY12}
        ...
        w1, w2 = SupAsymLimitPrices(b, c, cSup, True, lambsuphi, lambsuplo, lambrethi, lambretlo, LthetaR, LthetaS,
                           sens, fpr)
    elif eq == 5:  # {LHN12}
        w1, w2 = SupAsymLimitPrices(b, c, cSup, False, lambsuphi, lambsuplo, lambrethi, lambretlo, LthetaR, LthetaS,
                                    sens, fpr)
    elif eq == 6:  # {HHY1}
        w1, w2 = SupExclPrices(b, c, cSup, True, True, lambsuphi, lambsuplo, lambrethi, lambretlo, LthetaR, LthetaS, sens,
                               fpr, printUpdate=printUpdate, numWpts=numWpts)
    elif eq == 7:  # {HHN1}
        w1, w2 = SupExclPrices(b, c, cSup, True, False, lambsuphi, lambsuplo, lambrethi, lambretlo, LthetaR, LthetaS, sens,
                               fpr, printUpdate=printUpdate, numWpts=numWpts)
    elif eq == 8:  # {LLY1}
        w1, w2 = SupExclPrices(b, c, cSup, False, True, lambsuphi, lambsuplo, lambrethi, lambretlo, LthetaR, LthetaS, sens,
                               fpr, printUpdate=printUpdate, numWpts=numWpts)
    elif eq == 9:  # {LLN1}
        w1, w2 = SupExclPrices(b, c, cSup, False, False, lambsuphi, lambsuplo, lambrethi, lambretlo, LthetaR, LthetaS, sens,
                               fpr, printUpdate=printUpdate, numWpts=numWpts)
    elif eq == 10:  # {HLY1}
        w1, w2 = SupExclPrices(b, c, cSup, True, True, lambsuphi, lambsuplo, lambrethi, lambretlo, LthetaR, LthetaS, sens,
                               fpr, printUpdate=printUpdate, numWpts=numWpts)
...
def LthetaEqMatsForPlot(numLpts, LthetaR_max, LthetaS_max, numWpts, lambsuplo, lambsuphi, b, c, cSup, lambretlo, lambrethi, sens, fpr,
    ...
    eq_list = ['HHY12', 'HHN12', 'LLY12', 'LLN12', 'LHY12', 'LHN12', 'HHY1', 'HHN1', 'LLY1', 'LLN1', 'HLY1', 'HLN1',
               'LHY1', 'LHN1']
    retstrat_list = ['Y12', 'N12', 'Y1', 'N1', 'Y2', 'N2', 'N']
    eqStrat_matList = []
    eqQuant_matList = []
    ...
    Lr_insp_vec = LthetaR_vec[LthetaR_vec > Lr_insp_min]
    Lr_insp_vec = Lr_insp_vec[Lr_insp_vec < Lr_insp_max]
    Ls_insp_vec = LthetaS_vec[LthetaS_vec > Ls_insp_min]
    ...
    for eqind, eq_curr in enumerate(eq_list):
        ...
        if eqind < 4:  # Double-supplier symmetric sourcing
            # wholesale prices
        ...
        for Lrind, Lr in enumerate(LthetaR_vec):
            for Lsind, Ls in enumerate(LthetaS_vec):
                ...
                if (Lr_ind_Tmat < 0 and Ls_ind_Tmat < 0) or (Lrind == Lr_ind_Tmat and Lsind == Ls_ind_Tmat):
                    if eqind >= 4:  # Single-supplier or asymmetric sourcing
                        # wholesale prices
                    ...
                    if eq_curr in ['HHY12', 'HHN12', 'HHY1', 'HHN1']:
                        ...
                        s2util_eq = SupUtil(q2_eq, w2_eq, 0, Ls, lambret_eq, lambsup2_eq, sens, fpr)
                    if printUpdates is True:
                        print('US1, US2: ' + str(round(s1util_eq, 3)) + ', ' + str(round(s2util_eq, 3)))
                    ...
                    if bothSupsSourced is True:
                        ...
                        utilRetVec_eq = UtilsRet_Scen5(lambsup1_eq, lambsup2_eq, Lr, b, c, w1_eq, w2_eq, lambretlo,
                                                       ...
                                                       onlyQualInvest=onlyQualInvest)
                    if printUpdates is True:
                        print('Uret: ' + str(round(utilRetVec_eq[retStrat_eq], 3)))
...
numLpts = 15  # Refinement along each axis for plotting
numWpts = 30  # Refinement for deviation prices
LthetaR_max, LthetaS_max = 1.6, 0.8
LthetaR_vec = np.arange(0, LthetaR_max + 0.00001, LthetaR_max / numLpts)
LthetaS_vec = np.arange(0, LthetaS_max + 0.00001, LthetaS_max / numLpts)
...
eqStrat_matList, eqQuant_matList, _ = LthetaEqMatsForPlot(numLpts, LthetaR_max, LthetaS_max, numWpts, lambsuplo_0, lambsuphi_0, b_0,
...
for Lr_ind, Lr in enumerate(LthetaR_vec):
    for Ls_ind, Ls in enumerate(LthetaS_vec):
        if np.nansum([eqStrat_matList[i][Lr_ind, Ls_ind] for i in range(len(eqStrat_matList))]) == 0:
            # print('Lr:' + str(Lr_ind))
            # print('Ls:' + str(Ls_ind))
            # Need to identify equilibria mixture
            _, _, currTmat = LthetaEqMatsForPlot(numLpts, LthetaR_max, LthetaS_max, numWpts, lambsuplo_0, lambsuphi_0, b_0, c_0,
...
# Drop single-source overlaps with double-source equilibria if overlaps present
if doubleSourcePreempt is True:
    for Lr_ind, Lr in enumerate(LthetaR_vec):
        for Ls_ind, Ls in enumerate(LthetaS_vec):
            if np.nansum([eqStrat_matList[i][Lr_ind, Ls_ind] for i in range(6)]) > 0:
                for j in range(6, len(eqStrat_matList)):
                    eqStrat_matList[j][Lr_ind, Ls_ind] = np.nan
...
for eqind in range(len(labels)):
    ...
    if eqcolors[eqind] == 'black':  # No alpha transparency
        im = axStrat.imshow(eqStrat_matList[eqind].T, vmin=0, vmax=1,
                            extent=(LthetaR_vec.min(), LthetaR_vec.max(), LthetaS_vec.min(), LthetaS_vec.max()),
...
def sliders_on_changed(val):
    eqStrat_matList, eqQuant_matList, _ = LthetaEqMatsForPlot(numLpts, LthetaR_max, LthetaS_max, numWpts, lambsuplo_slider.val,
    ...
    for Lr_ind, Lr in enumerate(LthetaR_vec):
        for Ls_ind, Ls in enumerate(LthetaS_vec):
            if np.nansum([eqStrat_matList[i][Lr_ind, Ls_ind] for i in range(len(eqStrat_matList))]) == 0:
                ...
                # print('Ls:' + str(Ls_ind))
                # Need to identify equilibria mixture
                _, _, currTmat = LthetaEqMatsForPlot(numLpts, LthetaR_max, LthetaS_max, numWpts, lambsuplo_0,
                                                     lambsuphi_0, b_0, c_0,
                                                     cSup_0, lambretlo_0, lambrethi_0, sens_0, fpr_0,
                                                     printUpdates=False,
                                                     Lr_insp_min=Lr_insp_min, Lr_insp_max=Lr_insp_max,
                                                     Ls_insp_min=Ls_insp_min, Ls_insp_max=Ls_insp_max,
                                                     Lr_ind_Tmat=Lr_ind,
                                                     Ls_ind_Tmat=Ls_ind, onlyDoubleSource=doubleSource,
                                                     onlyQualInvest=retQualInvest)
                currTvec = steadyStateMat(currTmat)
...
eqStrat_matList, eqQuant_matList, Tmat = LthetaEqMatsForPlot(numLpts, LthetaR_max, LthetaS_max, numWpts, lambsuplo_0, lambsuphi_0,
...
bset = [0.25, 0.5, 0.9]
cset = [0.03, 0.08, 0.15, 0.3]
cSupset = [0.03, 0.08, 0.15, 0.3]
lambretloset, lambsuploset = [0.8], [0.8]

for b in bset:
    for c in cset:
        for cSup in cSupset:
            for lambretlo in lambretloset:
                for lambsuplo in lambsuploset:

                    numLpts = 12  # Refinement along each axis for plotting
                    numWpts = 30  # Refinement for deviation prices
                    LthetaR_max, LthetaS_max = 1.6, 0.8
                    LthetaR_vec = np.arange(0, LthetaR_max + 0.00001, LthetaR_max / numLpts)
                    LthetaS_vec = np.arange(0, LthetaS_max + 0.00001, LthetaS_max / numLpts)
                    ...
                    eqStrat_matList, eqQuant_matList, _ = LthetaEqMatsForPlot(numLpts, LthetaR_max, LthetaS_max, numWpts, lambsuplo, lambsuphi_0, b,
                                                                              c, cSup, lambretlo, lambrethi_0, sens_0, fpr_0,
                                                                              printUpdates=False, Lr_insp_min=Lr_insp_min,
                                                                              Lr_insp_max=Lr_insp_max, Ls_insp_min=Ls_insp_min,
                                                                              Ls_insp_max=Ls_insp_max, onlyDoubleSource=doubleSource,
                                                                              onlyQualInvest=retQualInvest)
                    ...
                    for Lr_ind, Lr in enumerate(LthetaR_vec):
                        for Ls_ind, Ls in enumerate(LthetaS_vec):
                            if np.nansum([eqStrat_matList[i][Lr_ind, Ls_ind] for i in range(len(eqStrat_matList))]) == 0:
                                ...
                                # print('Ls:' + str(Ls_ind))
                                # Need to identify equilibria mixture
                                _, _, currTmat = LthetaEqMatsForPlot(numLpts, LthetaR_max, LthetaS_max, numWpts, lambsuplo, lambsuphi_0, b,
                                                                              c, cSup, lambretlo, lambrethi_0, sens_0, fpr_0, printUpdates=False,
                                                                     Lr_insp_min=Lr_insp_min, Lr_insp_max=Lr_insp_max,
                                                                     Ls_insp_min=Ls_insp_min, Ls_insp_max=Ls_insp_max, Lr_ind_Tmat=Lr_ind,
                                                                     Ls_ind_Tmat=Ls_ind, onlyDoubleSource=doubleSource,
                                                                     onlyQualInvest=retQualInvest)
                    ...
                    fig = plt.figure()
                    fig.suptitle('Nash equilibria vs. '+r'$L^{R}_{\theta}$'+', ' +r'$L^{S}_{\theta}$'+
                                 '\nb='+str(b)+', c='+str(c)+', cSup='+str(cSup)+', lamRetLo='+str(lambretlo) +
                                 ', lambSupLo='+str(lambsuplo), fontsize=18, fontweight='bold')
                    axStrat = fig.add_subplot(321)
                    axQuant = fig.add_subplot(322)
                    axSupQual = fig.add_subplot(323)
                    axRetQual = fig.add_subplot(324)
                    ...
                    slwidth, slht = 0.2, 0.02
                    b_slider_ax = fig.add_axes([slstrtrightval, sltopval, slwidth, slht])
                    b_slider = Slider(b_slider_ax, r'$b$', 0.01, 0.99, valinit=b)
                    c_slider_ax = fig.add_axes([slstrtrightval, sltopval-slgap, slwidth, slht])
                    c_slider = Slider(c_slider_ax, r'$c_R$', 0.01, 0.99, valinit=c)
                    cSup_slider_ax = fig.add_axes([slstrtrightval, sltopval-slgap*2, slwidth, slht])
                    cSup_slider = Slider(cSup_slider_ax, r'$c_S$', 0.01, 0.99, valinit=cSup)
                    lambretlo_slider_ax = fig.add_axes([slstrtrightval, sltopval-slgap*3, slwidth, slht])
                    lambretlo_slider = Slider(lambretlo_slider_ax, r'$\lambda^{lo}$', 0.01, 0.99, valinit=lambretlo)
                    lambrethi_slider_ax = fig.add_axes([slstrtrightval, sltopval-slgap*4, slwidth, slht])
                    lambrethi_slider = Slider(lambrethi_slider_ax, r'$\lambda^{hi}$', 0.01, 0.99, valinit=lambrethi_0)
                    lambsuplo_slider_ax = fig.add_axes([slstrtrightval, sltopval-slgap*5, slwidth, slht])
                    lambsuplo_slider = Slider(lambsuplo_slider_ax, r'$\Lambda^{lo}$', 0.01, 0.99, valinit=lambsuplo)
                    lambsuphi_slider_ax = fig.add_axes([slstrtrightval, sltopval-slgap*6, slwidth, slht])
                    lambsuphi_slider = Slider(lambsuphi_slider_ax, r'$\Lambda^{hi}$', 0.01, 0.99, valinit=lambsuphi_0)
                    sens_slider_ax = fig.add_axes([slstrtrightval, sltopval-slgap*7, slwidth, slht])
                    sens_slider = Slider(sens_slider_ax, r'$\rho$', 0.5, 0.99, valinit=sens_0)
